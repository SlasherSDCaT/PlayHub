...
const Awards = () => {
    ...
    const handleDeleteAward = async (awardId) => {
        try {
            const response = await axios.delete(`http://localhost:8000/awards/${awardId}`);
            if (response.status === 200) {
                fetchData(); // Запросить обновленные данные после успешного удаления награды
            } else {
                console.error('Ошибка при удалении награды');
            }
        } catch (error) {
            console.error('Ошибка при выполнении запроса:', error);
        }
    ...
    const handleCreateAward = async () => {
        try {
            const response = await axios.post('http://localhost:8000/awards/', newAwardData);
            if (response.status === 200) {
                ...
                <>
                    ...
                    <Modal show={showModal} onHide={() => setShowModal(false)}>
                        <Modal.Header closeButton>
                            <Modal.Title>Создать награду</Modal.Title>
                        </Modal.Header>
                        <Modal.Body>
                            <Form>
                                ...
                                <Form.Group controlId="formExperience">
                                    ...
                                    <Form.Control
                                        type="number"
                                        placeholder="Введите количество очков опыта"
                                        name="experience_points"
                                        value={newAwardData.experience_points}
                                        onChange={handleInputChange}
                                    />
                                </Form.Group>
                            </Form>
